...
export default function Navbar( val ) {
    ...
    //closes drop nav with outside click
    const [changed, setChanged] = useState(0)
    const toggleChange = () => {
        changed ? setChanged(0) : setChanged(1)
    }
    useEffect(() => {
        document.getElementById('off').addEventListener('click', toggleChange)
        if(toggle)
            document.getElementById('off').addEventListener('click', setToggle(0))
        return () => {
            document.getElementById('off').removeEventListener('click', toggleChange)
        }
    },[changed])
